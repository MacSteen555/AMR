...
export default function CompetitiveDashboard() {
    ...
    // Configuration panel (for changing locations/competitors in refresh)
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // Auto-select locations/competitors from most recent run, or default to all
            ...
            if (latestRun) {
                ...
                // Default: select all locations (up to 3) and all competitors (up to 3)
    ...
    // The currently displayed run (latest or selected from history)
    const activeRun = useMemo(() => {
        if (selectedRunId) return runs.find(r => r.id === selectedRunId) || runs[0]
        return runs[0] || null
    }, [runs, selectedRunId])

    // Free stats computed from competitor + location data
    const quickStats = useMemo(() => {
        const compWithRatings = competitors.filter(c => c.rating)
        const avgCompRating = compWithRatings.length
            ? compWithRatings.reduce((sum, c) => sum + (c.rating || 0), 0) / compWithRatings.length
            : null
        const totalCompReviews = competitors.reduce((sum, c) => sum + (c.review_count || 0), 0)

        // Use hero metrics from latest run if available
    ...
    const handleDeleteRun = async (id: string) => {
        ...
        try {
            ...
            setToast({ message: err.message || 'Failed to delete analysis', type: 'error' })
        }
    }
        ...
        <>
            ...
            <div className="p-8 h-screen flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto space-y-8 pb-20">
                    ...
                    {competitors.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {competitors.map(comp => (
                                <div key={comp.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col relative group">
                                    <div className="flex justify-between items-start mb-3">
                                        <h3 className="text-base font-bold text-gray-900 truncate pr-2">{comp.name}</h3>
                                        <button onClick={() => handleDeleteCompetitor(comp.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all shrink-0" title="Delete">
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                        </button>
                                    </div>

                                    {comp.address && (
                                        <p className="text-xs text-gray-500 mb-3 truncate">{comp.address}</p>
                                    )}
                    ...
                    {/* ── Quick Stats Bar (Free — computed from existing data) ── */}
                    {competitors.length > 0 && (
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">
                                        {quickStats.yourRating ? quickStats.yourRating.toFixed(1) : '—'}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400 mb-1">Competitor Avg Rating</p>
                                    <p className="text-2xl font-bold text-gray-900">
                                        {quickStats.competitorRating ? quickStats.competitorRating.toFixed(1) : '—'}
                                    </p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">
                                        {quickStats.yourReviews !== null ? quickStats.yourReviews.toLocaleString() : '—'}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400 mb-1">Competitor Reviews</p>
                                    <p className="text-2xl font-bold text-gray-900">
                                        {quickStats.competitorReviews ? quickStats.competitorReviews.toLocaleString() : '—'}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {competitors.length > 0 && (
                        <div className="space-y-4">
                                    ...
                                    {runs.length > 1 && (
                                        <div className="relative">
                                            <button
                                                ...
                                                className="px-3 py-2 text-sm text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-1.5"
                                            >
                                                ...
                                                <div className="absolute right-0 mt-1 w-72 bg-white border border-gray-200 rounded-xl shadow-lg z-20 max-h-64 overflow-y-auto">
                                                    {runs.map((run, i) => (
                                                        <button
                                                            ...
                                                            className={`w-full text-left px-4 py-3 hover:bg-gray-50 border-b border-gray-100 last:border-0 flex items-center justify-between transition-colors ${selectedRunId === run.id ? 'bg-teal-50' : ''}`}
                                                        >
                                                            <div>
                                                                <span className="text-sm font-medium text-gray-900">
                                                                    {i === 0 ? 'Latest analysis' : run.name || `Analysis ${runs.length - i}`}
                                                                </span>
                            ...
                            {/* Config panel (shown when "Configure" is clicked) */}
                            {showConfig && (
                                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                                    <div className="flex justify-between items-center mb-4">
                                        <h3 className="font-semibold text-gray-900">Change Comparison</h3>
                            ...
                            {activeRun && activeRun.data ? (
                                ...
                                // No runs yet — prompt to create first analysis
                                <div className="text-center py-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
