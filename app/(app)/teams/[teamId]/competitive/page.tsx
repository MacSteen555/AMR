...
export default function CompetitiveDashboard() {
    ...
    const [editingGroup, setEditingGroup] = useState<CompetitiveGroup | null>(null)
    const [groupName, setGroupName] = useState('')
    const [groupLocations, setGroupLocations] = useState<string[]>([])
    const [groupCompetitors, setGroupCompetitors] = useState<string[]>([])
    ...
    // Runs filtered to active group
    const groupRuns = useMemo(() => {
        if (!activeGroupId) return runs
        return runs.filter(r => r.group_id === activeGroupId)
    }, [runs, activeGroupId])

    const activeRun = useMemo(() => {
        if (selectedRunId) return groupRuns.find(r => r.id === selectedRunId) || groupRuns[0]
        return groupRuns[0] || null
    }, [groupRuns, selectedRunId])

    // Competitors in the active group
    const groupCompetitorsList = useMemo(() => {
        if (!activeGroup) return competitors
        return competitors.filter(c => activeGroup.competitor_ids.includes(c.id))
    }, [competitors, activeGroup])

    // Quick stats scoped to active group's competitors
    const quickStats = useMemo(() => {
        const relevantComps = groupCompetitorsList
        const compWithRatings = relevantComps.filter(c => c.rating)
        const avgCompRating = compWithRatings.length
            ? compWithRatings.reduce((sum, c) => sum + (c.rating || 0), 0) / compWithRatings.length
            : null
        const totalCompReviews = relevantComps.reduce((sum, c) => sum + (c.review_count || 0), 0)
        ...
        return {
            ...
            competitorRating: avgCompRating,
            competitorReviews: totalCompReviews,
            ratingGap: ownAvgRating && avgCompRating ? ownAvgRating - avgCompRating : null,
        }
    }, [groupCompetitorsList, activeRun])
    ...
    const loadData = async () => {
        setLoading(true)
        try {
            const [compRes, groupRes, runRes, locRes] = await Promise.all([
                apiGet<{ competitors: Competitor[] }>(`/api/teams/${teamId}/competitors`),
                apiGet<{ groups: CompetitiveGroup[] }>(`/api/teams/${teamId}/competitive-groups`),
            ...
            // Select default group or first group
            const allGroups = groupRes.groups || []
            if (allGroups.length > 0 && !activeGroupId) {
    ...
    const openGroupForm = (group?: CompetitiveGroup) => {
        if (group) {
            setEditingGroup(group)
            setGroupName(group.name)
            setGroupLocations(group.owned_location_ids || [])
            setGroupCompetitors(group.competitor_ids || [])
        } else {
            setEditingGroup(null)
            setGroupName('')
            setGroupLocations(locations.slice(0, 3).map(l => l.id))
            setGroupCompetitors([])
        }
    ...
    const handleSaveGroup = async () => {
        ...
        try {
            if (editingGroup) {
                await apiPatch(`/api/teams/${teamId}/competitive-groups/${editingGroup.id}`, {
                    name: groupName,
                    owned_location_ids: groupLocations,
                    competitor_ids: groupCompetitors,
                })
                setToast({ message: 'Group updated', type: 'success' })
            } else {
                const res = await apiPost<{ group: CompetitiveGroup }>(`/api/teams/${teamId}/competitive-groups`, {
                    name: groupName,
                    owned_location_ids: groupLocations,
                    competitor_ids: groupCompetitors,
                })
            ...
            setToast({ message: err.message || 'Failed to save group', type: 'error' })
        } finally { setIsSubmitting(false) }
    }

    const handleDeleteGroup = async (groupId: string) => {
        if (!confirm('Delete this group? Analysis history for this group will be unlinked.')) return
        try {
            await apiDelete(`/api/teams/${teamId}/competitive-groups/${groupId}`)
            if (activeGroupId === groupId) setActiveGroupId(null)
            setToast({ message: 'Group deleted', type: 'success' })
            loadData()
        } catch (err: any) { setToast({ message: err.message || 'Failed to delete group', type: 'error' }) }
    }
    ...
    const handleRefreshAnalysis = async () => {
        if (!activeGroup) {
            setToast({ message: 'Please create a group first', type: 'error' })
            return
        }
        if (activeGroup.owned_location_ids.length === 0 || activeGroup.competitor_ids.length === 0) {
            setToast({ message: 'This group needs at least 1 location and 1 competitor. Edit the group to add them.', type: 'error' })
            return
        }
        ...
        try {
            await apiPost(`/api/teams/${teamId}/competitive-runs`, {
                ...
                owned_location_ids: activeGroup.owned_location_ids,
                competitor_ids: activeGroup.competitor_ids,
            })
            ...
            <div className="p-8 h-screen flex flex-col">
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setShowAddForm(true)} className="px-4 py-2 text-sm bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition-colors">
                            Add Competitor
                        </button>
                        <button onClick={() => openGroupForm()} className="px-4 py-2 text-sm bg-teal-600 text-white rounded-lg hover:bg-teal-700 font-medium transition-colors">
                            New Group
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-6 pb-20">
                    ...
                    {groups.length > 0 && (
                        <div className="flex items-center gap-2 overflow-x-auto pb-1">
                            {groups.map(group => (
                                ...
                                >
                                    {group.name}
                                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeGroupId === group.id ? 'bg-teal-500 text-teal-100' : 'bg-gray-100 text-gray-500'}`}>
                                        {group.competitor_ids.length}
                                    </span>
                                    {/* Edit/delete on hover */}
                                    <span className={`items-center gap-1 ml-1 ${activeGroupId === group.id ? 'flex' : 'hidden group-hover/tab:flex'}`}>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); openGroupForm(group) }}
                                            className={`p-0.5 rounded ${activeGroupId === group.id ? 'hover:bg-teal-500 text-teal-200' : 'hover:bg-gray-200 text-gray-400'}`}
                                            title="Edit group"
                                        >
                    ...
                    {showGroupForm && (
                        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                            <div className="flex justify-between items-center mb-5">
                                <h3 className="text-lg font-bold text-gray-900">{editingGroup ? 'Edit Group' : 'Create Competitor Group'}</h3>
                                <button onClick={() => { setShowGroupForm(false); setEditingGroup(null) }} className="text-gray-400 hover:text-gray-600">
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-500 shadow-sm"
                                        placeholder='e.g. "Premium Competitors" or "Downtown Rivals"'
                                        autoFocus
                                    />
                                </div>
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                                        <h4 className="font-medium text-gray-700 mb-3 flex items-center justify-between text-sm">
                                            ...
                                            <span className="text-xs bg-white text-teal-600 px-2 py-0.5 rounded border border-teal-100">{groupLocations.length}/3</span>
                                        </h4>
                                            ...
                                            {locations.map(loc => (
                                                <label key={loc.id} className="flex items-center gap-3 bg-white p-2.5 rounded-lg border border-gray-100 cursor-pointer hover:border-teal-200 transition-colors">
                                                    <input
                                                        ...
                                                        checked={groupLocations.includes(loc.id)}
                                                        onChange={(e) => {
                                                            if (e.target.checked && groupLocations.length < 3) setGroupLocations([...groupLocations, loc.id])
                                                            else if (!e.target.checked) setGroupLocations(groupLocations.filter(id => id !== loc.id))
                                                        }}
                                                        disabled={!groupLocations.includes(loc.id) && groupLocations.length >= 3}
                                                    />
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                                        <h4 className="font-medium text-gray-700 mb-3 flex items-center justify-between text-sm">
                                            ...
                                            <span className="text-xs bg-white text-rose-600 px-2 py-0.5 rounded border border-rose-100">{groupCompetitors.length}/3</span>
                                        </h4>
                                        <div className="space-y-2 max-h-40 overflow-y-auto">
                                            {competitors.map(comp => (
                                                <label key={comp.id} className="flex items-center gap-3 bg-white p-2.5 rounded-lg border border-gray-100 cursor-pointer hover:border-rose-200 transition-colors">
                                                    <input
                                                        ...
                                                        checked={groupCompetitors.includes(comp.id)}
                                                        onChange={(e) => {
                                                            if (e.target.checked && groupCompetitors.length < 3) setGroupCompetitors([...groupCompetitors, comp.id])
                                                            else if (!e.target.checked) setGroupCompetitors(groupCompetitors.filter(id => id !== comp.id))
                                                        }}
                                                        disabled={!groupCompetitors.includes(comp.id) && groupCompetitors.length >= 3}
                                                    />
                                                    <span className="text-sm text-gray-800">{comp.name}</span>
                                                </label>
                                            ))}
                                            {competitors.length === 0 && <p className="text-sm text-gray-500 italic">No competitors added yet.</p>}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex justify-end gap-2 pt-2">
                                    <button onClick={() => { setShowGroupForm(false); setEditingGroup(null) }} className="px-5 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors">Cancel</button>
                                    <button onClick={handleSaveGroup} disabled={!groupName.trim() || isSubmitting} className="px-5 py-2 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 disabled:opacity-50 transition-colors shadow-sm">
                    ...
                    {/* ── No groups yet — onboarding ── */}
                    {groups.length === 0 && competitors.length === 0 && !showAddForm && !showGroupForm && (
                        <div className="text-center py-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                            <p className="text-gray-500 mb-4">No competitors tracked yet. Add competitors, then organize them into groups to compare.</p>
                    ...
                    {groups.length === 0 && competitors.length > 0 && !showGroupForm && (
                        <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                            <p className="text-gray-500 mb-4">You have {competitors.length} competitor{competitors.length > 1 ? 's' : ''} tracked. Create a group to start comparing.</p>
                            <button onClick={() => openGroupForm()} className="px-4 py-2 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700">Create First Group</button>
                        </div>
                    )}
                    ...
                    {activeGroup && (
                        <>
                            {/* Competitor cards for this group */}
                            {groupCompetitorsList.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                    {groupCompetitorsList.map(comp => (
                                        <div key={comp.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col relative group/card">
                                            <div className="flex justify-between items-start mb-3">
                                                <h3 className="text-base font-bold text-gray-900 truncate pr-2">{comp.name}</h3>
                                                <button onClick={() => handleDeleteCompetitor(comp.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover/card:opacity-100 transition-all shrink-0" title="Delete">
                                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                </button>
                                            </div>
                                            {comp.address && <p className="text-xs text-gray-500 mb-3 truncate">{comp.address}</p>}
                                            <div className="flex items-center gap-3 mb-4">
                                                {comp.rating && (
                                                    <span className="flex items-center gap-1 text-sm font-medium text-amber-600">
                                                        <span className="text-amber-500">★</span> {comp.rating.toFixed(1)}
                                                        {comp.review_count ? <span className="text-gray-400 font-normal text-xs">({comp.review_count.toLocaleString()})</span> : null}
                                                    </span>
                                                )}
                                            </div>
                                            <div className="mt-auto flex items-center justify-between pt-3 border-t border-gray-100">
                                                <span className={`text-xs font-medium ${comp.last_serp_sync_status === 'syncing' ? 'text-blue-500 animate-pulse' : comp.last_serp_sync_status === 'error' ? 'text-red-500' : 'text-gray-400'}`}>
                                                    {comp.last_serp_sync_status === 'syncing' ? 'Syncing...' : comp.last_serp_sync_status === 'error' ? 'Sync failed' : comp.last_serp_sync_at ? `Synced ${new Date(comp.last_serp_sync_at).toLocaleDateString()}` : 'Never synced'}
                                                </span>
                                                <div className="flex gap-1.5">
                                                    <button onClick={() => setSelectedCompetitorStats(comp)} className="p-1.5 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="View Details">
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                                                    </button>
                                                    <button onClick={() => handleSyncReviews(comp.id)} disabled={comp.last_serp_sync_status === 'syncing'} className="p-1.5 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors disabled:opacity-50" title="Fetch Reviews">
                                ...
                                <div className="text-center py-10 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                                    <p className="text-gray-500 mb-3">No competitors in this group yet.</p>
                                    <button onClick={() => openGroupForm(activeGroup)} className="px-4 py-2 bg-teal-100 text-teal-700 rounded-lg font-medium hover:bg-teal-200 text-sm">Edit Group</button>
                                </div>
                            )}
                            ...
                            {groupCompetitorsList.length > 0 && (
                                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                                        ...
                                        <div>
                                            <p className="text-xs text-gray-400 mb-1">Competitor Avg Rating</p>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                                    ...
                                                    {showHistory && (
                                                        <div className="absolute right-0 mt-1 w-72 bg-white border border-gray-200 rounded-xl shadow-lg z-20 max-h-64 overflow-y-auto">
                                                            {groupRuns.map((run, i) => (
                                                                <button
                                                                    key={run.id}
                                                                    onClick={() => { setSelectedRunId(run.id); setShowHistory(false) }}
                                                                ...
                                                                >
                                                                    <div>
                                                                        <span className="text-sm font-medium text-gray-900">{i === 0 ? 'Latest analysis' : `Analysis ${groupRuns.length - i}`}</span>
                                            ...
                                            <button onClick={() => openGroupForm(activeGroup)} className="px-3 py-2 text-sm text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-1.5">
                                            ...
                                            <button
                                                ...
                                                disabled={isRefreshing || activeGroup.owned_location_ids.length === 0 || activeGroup.competitor_ids.length === 0}
                                    ...
                                    {activeRun && activeRun.data ? (
                                        ...
                                        <div className="text-center py-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                                            ...
                                            <h3 className="text-lg font-bold text-gray-900 mb-2">No analysis for &ldquo;{activeGroup.name}&rdquo;</h3>
                                            <p className="text-gray-500 mb-6 max-w-md mx-auto">Run your first AI-powered competitive analysis for this group.</p>
                                            <button
                                                onClick={handleRefreshAnalysis}
                                                disabled={isRefreshing || activeGroup.owned_location_ids.length === 0 || activeGroup.competitor_ids.length === 0}
